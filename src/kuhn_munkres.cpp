...
#else // C++11
            float max_element = -__FLT_MAX__;
            for(kuhn_munkres::Vec1f::iterator it = row.begin(); it < row.end(); ++it)
                max_element = fmax(max_element, *it);
            maxinum = fmax(maxinum, max_element);
#endif
        }
...
int KuhnMunkres::step1() {
    for (int i = 0; i < this->n; ++i) {
        ...
        for(kuhn_munkres::Vec1f::iterator it = this->C[i].begin(); it < this->C[i].end(); ++it)
            minVal = fmin(minVal, *it);
#endif
...
#else // C++11
        pair<int, int> P = findAZero(row, col);
        int r = P.first;
        int c = P.second;
#endif
        row = r, col = c;
